...
<body class="flex items-center justify-center min-h-screen p-4">
    <div class="bg-white p-8 md:p-12 rounded-3xl shadow-2xl max-w-xl w-full text-center">
        ...
        <div class="space-y-6">
            ...
            <!-- Use the combined route to ensure the base path is covered -->
...
def proxy_data_entry_web(path):
    ...
    """
    # 基础 URL: http://DATA_ENTRY_HOST:DATA_ENTRY_PORT
    base_url = f"http://{DATA_ENTRY_HOST}:{DATA_ENTRY_PORT}"
    
    # 目标路径: DATA_ENTRY_PATH (如 /web) 加上从 URL 捕获的路径 (path)
    # urljoin 确保路径拼接正确，避免双斜杠问题
    internal_path = urljoin(DATA_ENTRY_PATH, path)
    
    # 完整的内部目标 URL
    target_url = urljoin(base_url, internal_path)
    ...
    try:
        ...
        if method == 'POST':
            # For POST requests, we forward the raw request body
